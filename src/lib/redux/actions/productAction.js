...
export const getProducts = createAsyncThunk(
    "get/products", async(queryData,{rejectWithValue})=>{
        try {
            const {id, minPrice=0, maxPrice=0, from,page} = queryData;
            const config ={
                headers:{
                    "Content-Type":"application/json"
                }
            }
            const { data }  = await axios.get(`/api/product?categoryId=${id}&minPrice=${minPrice}&maxPrice=${maxPrice}&from=${from}&page=${page}`,config)
